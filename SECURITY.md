...
3. **`API_KEY` for Home Assistant only** — Required for `GET /devices` and `GET /?json=true`. Fail closed when unset. Clients must send `Authorization: Bearer` or `x-api-key` (query-string keys are not accepted). After the `/devices` bypass, `API_KEY` is the sole guard on that endpoint — use a strong secret and rotate if exposed. `GET /?json=true` remains on path `/` behind the dashboard Access application; do not Bypass `/`.
...
5. **Protect your webhook URL** — Alert settings are stored in KV. Use a secret ntfy topic name or private Apprise endpoint; anyone who knows that URL can send notifications to it directly. The dashboard redacts stored webhook paths in HTML/API responses. Alert latching is best-effort: overlapping cron invocations can still double-send because KV is non-atomic.
6. **Dashboard mutation throttles** — `POST /alert-config` and `POST /test-alert` use a soft KV rate limit (fail-open, shared bucket, non-atomic). Treat it as UX only. For abuse prevention, add Cloudflare Rate Limiting / WAF rules on those routes.
7. **Rotate secrets** — If `API_KEY`, `ALLOWED_EMAIL_TO`, or your Cloudflare API token is exposed, rotate them immediately in the Cloudflare dashboard and GitHub repository secrets.
8. **Limit API token scope** — Use a Cloudflare API token scoped only to the Workers, KV, and (if managing Access via API) Access Apps and Policies resources this project needs.
