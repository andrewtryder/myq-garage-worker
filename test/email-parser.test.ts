...
describe('email-parser unit tests', () => {
  ...
  describe('resolveFeedKey', () => {
    it('resolves right garage door to right feed', () => {
      const key = resolveFeedKey('Garage Door Right', mockEnv);
      expect(key).toBe('right-feed-key');
    });

    it('resolves left garage door to left feed', () => {
      const key = resolveFeedKey('Garage Door Left', mockEnv);
      expect(key).toBe('left-feed-key');
    });

    it('returns null for unknown device name', () => {
      const key = resolveFeedKey('Front Gate', mockEnv);
      expect(key).toBeNull();
    });
  });
